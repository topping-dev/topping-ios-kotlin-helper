...
//--------------------------------------------------------------------------------------------------
    ...
    override operator fun get(key: Long): E? =
    ...
    operator fun get(key: Long, defaultValue: E): E = getOrDefault(key, defaultValue)

    fun getOrDefault(key: Long, defaultValue: E): E =
        _keys.binarySearch(key, toIndex = _size).let { i ->
            if (i < 0) {
                defaultValue
            } else {
    ...
    override val keys: MutableSet<Long>
        get() {
            ...
            return LinkedHashSet<Long>(_size).also {
    ...
    override val values: MutableList<E>
        get() {
            ...
            return MutableList(_size) {
                _values[it] as E
            }
        }

    override val entries: MutableSet<MutableMap.MutableEntry<Long, E>>
        get() {
            if (garbage) gc()

            return LinkedHashSet<MutableMap.MutableEntry<Long, E>>(_size).also {
                for (i in 0 until _size) {
     ...
     * was one.
     */
    override fun put(key: Long, value: E): E? {
     ...
     */
    override fun remove(key: Long): E? =
        _keys.binarySearch(key, toIndex = _size).let { i ->
     ...
     */
    fun remove(key: Long, valueToRemove: E): Boolean {
    ...
    override fun putAll(from: Map<out Long, E>) {
        for (item in from) {
...
//--------------------------------------------------------------------------------------------------
    ...
    override fun iterator(): MutableIterator<Map.Entry<Long, E>> {
        if (garbage) gc()
        return object : MutableIterator<Map.Entry<Long, E>> {
            var i: Int = 0

            override fun hasNext(): Boolean = i < _size

            override fun next(): Map.Entry<Long, E> {
    ...
    ) : MutableMap.MutableEntry<Long, E> {
