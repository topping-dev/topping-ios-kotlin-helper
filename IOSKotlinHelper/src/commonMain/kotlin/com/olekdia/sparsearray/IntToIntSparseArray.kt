...
class IntToIntSparseArray(
    ...
    init {
        if (initialCapacity == 0) {
            ...
            _values = IntArray(initialCapacity)
        }
        _size = 0
    }
...
//--------------------------------------------------------------------------------------------------
    ...
    override fun containsValue(value: Int): Boolean = indexOfValue(value) >= 0
    ...
    operator fun get(key: Int, defaultValue: Int): Int = getOrDefault(key, defaultValue)
    ...
    override val entries: MutableSet<MutableMap.MutableEntry<Int, Int>>
        get() = LinkedHashSet<MutableMap.MutableEntry<Int, Int>>(_size).also {
            for (i in 0 until _size) {
    ...
    override fun remove(key: Int): Int? =
        _keys.binarySearch(key, toIndex = _size).let { i ->
            if (i >= 0) {
                val removedValue: Int = _values[i]
     ...
     */
    fun remove(key: Int, valueToRemove: Int): Boolean =
        _keys.binarySearch(key, toIndex = _size).let { i ->
...
//--------------------------------------------------------------------------------------------------
    ...
    override fun iterator(): MutableIterator<Map.Entry<Int, Int>> {
        return object : MutableIterator<Map.Entry<Int, Int>> {
            var i: Int = 0
    ...
    val valuesIterator: Iterator<Int>
        get() = object : Iterator<Int> {
            var i: Int = 0

            override fun hasNext(): Boolean = i < _size
            override fun next(): Int {
                val value: Int = _values[i]
                i++
                return value
            }
        }
