...
class Oscillator {
    ...
    override fun toString(): String {
        return "pos =" + Arrays.toString(mPosition).toString() + " period=" + Arrays.toString(
            mPeriod
        )
    }
    ...
    fun addPoint(position: Double, period: Float) {
        ...
        var j: Int = Arrays.binarySearch(mPosition, position)
        if (j < 0) {
            j = -j - 1
        }
        ...
        mPosition.copyInto(mPosition, j + 1, j, len - 1)
    ...
    fun getP(time: Double): Double {
        ...
        var index: Int = Arrays.binarySearch(mPosition, time)
        var p = 0.0
        if (index > 0) {
            p = 1.0
        } else if (index != 0) {
            ...
            p = (mArea[index - 1]
                    + (mPeriod[index - 1] - m * mPosition[index - 1]) * (t - mPosition[index - 1]) + m * (t * t - mPosition[index - 1] * mPosition[index - 1]) / 2)
        }
        return p
    }
    ...
    fun getSlope(time: Double, phase: Double, dphase: Double): Double {
        ...
        return when (mType) {
            SIN_WAVE -> mPI2 * dangle_dtime * cos(
                mPI2 * angle
            )
            SQUARE_WAVE -> 0.0
            TRIANGLE_WAVE -> 4 * dangle_dtime * signum(
                (angle * 4 + 3) % 4 - 2
            )
            SAW_WAVE -> dangle_dtime * 2
            REVERSE_SAW_WAVE -> -dangle_dtime * 2
            COS_WAVE -> -mPI2 * dangle_dtime * sin(
                mPI2 * angle
            )
            BOUNCE -> 4 * dangle_dtime * ((angle * 4 + 2) % 4 - 2)
            CUSTOM -> mCustomCurve!!.getSlope(
                angle % 1,
                0
            )
