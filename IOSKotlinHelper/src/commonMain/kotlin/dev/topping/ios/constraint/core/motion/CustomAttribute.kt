...
class CustomAttribute {
    ...
    fun setValue(value: Any) {
        when (type) {
            AttributeType.REFERENCE_TYPE, AttributeType.INT_TYPE -> mIntegerValue = value.toString().toInt()
            AttributeType.FLOAT_TYPE -> mFloatValue = value.toString().toFloat()
            AttributeType.COLOR_TYPE, AttributeType.COLOR_DRAWABLE_TYPE -> mColorValue =
                value.toString().toInt()
            AttributeType.STRING_TYPE -> mStringValue = value.toString()
            AttributeType.BOOLEAN_TYPE -> mBooleanValue = value.toString().toBoolean()
            AttributeType.DIMENSION_TYPE -> mFloatValue = value.toString().toFloat()
            else -> {}
        }
    }
