...
class HyperSpline {
    ...
    var mCurveLength: DoubleArray = DoubleArray(0)
    ...
    fun getVelocity(p: Double, v: DoubleArray) {
        ...
        for (i in v.indices) {
            v[i] = mCurve[i]?.get(k)!!.vel(pos / mCurveLength[k])
        }
    }
    ...
    fun getPos(p: Double, x: DoubleArray) {
        ...
        for (i in x.indices) {
            x[i] = mCurve[i]?.get(k)!!.eval(pos / mCurveLength[k])
        }
    }
    ...
    fun getPos(p: Double, x: FloatArray) {
        ...
        for (i in x.indices) {
            x[i] = mCurve[i]?.get(k)!!.eval(pos / mCurveLength[k]).toFloat()
        }
    }
    ...
    fun getPos(p: Double, splineNumber: Int): Double {
        ...
        return mCurve[splineNumber]?.get(k)!!.eval(pos / mCurveLength[k])
    }
