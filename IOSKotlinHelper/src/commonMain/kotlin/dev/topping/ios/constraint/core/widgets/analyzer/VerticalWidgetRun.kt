...
class VerticalWidgetRun(widget: ConstraintWidget) : WidgetRun(widget) {
    var baseline: DependencyNode = DependencyNode(this)
    var mBaselineDimension: DimensionDependency? =
        null
    ...
    fun update(dependency: Dependency) {
        ...
        if (!mDimension.resolved
            && mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT && mWidget.mMatchConstraintDefaultWidth == ConstraintWidget.MATCH_CONSTRAINT_SPREAD && !mWidget.isInVerticalChain
        ) {
    ...
    override fun apply() {
        ...
        if (mDimension.resolved && mWidget.measured) {
            ...
            if (!mDimension.resolved && mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                when (mWidget.mMatchConstraintDefaultHeight) {
                    ConstraintWidget.MATCH_CONSTRAINT_RATIO -> {
                        if (!mWidget.isInVerticalChain) {
                ...
                if (target != null) {
                    ...
                    if (mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                        if (mWidget.dimensionRatio > 0) {
                            if (mWidget.mHorizontalRun!!.mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
