 ...
 */
class ArrayLinkedVariables internal constructor(arrayRow: ArrayRow, cache: Cache) :
    ...
    override fun use(definition: ArrayRow?, removeFromDefinition: Boolean): Float {
        ...
        for (i in 0 until definitionSize) {
            ...
            val definitionValue: Float = definitionVariables?.get(definitionVariable) ?: 0f
    ...
    override fun clear() {
        ...
        var counter = 0
        while (current != NONE && counter < currentSize) {
            val variable: SolverVariable? = mCache.mIndexedVariables.get(mArrayIndices[current])
            variable?.removeFromRow(mRow)
            current = mArrayNextIndices[current]
            counter++
        }
...
// if no candidate is known, let's figure it out
     ...
     */
    val pivotCandidate: dev.topping.ios.constraint.core.SolverVariable?
        get() {
            if (mCandidate == null) {
                // if no candidate is known, let's figure it out
                var current = head
                ...
                while (current != NONE && counter < currentSize) {
                    if (mArrayValues[current] < 0) {
                        // We can return the first negative candidate as in ArrayLinkedVariables
                        // they are already sorted by id
                        val v: SolverVariable? = mCache.mIndexedVariables.get(mArrayIndices[current])
                        if (v != null && (pivot == null || pivot.strength < v.strength)) {
                            pivot = v
                        }
                    }
                    current = mArrayNextIndices[current]
    ...
    override fun getVariable(index: Int): SolverVariable? {
        ...
        while (current != NONE && counter < currentSize) {
            if (counter == index) {
                return mCache.mIndexedVariables.get(mArrayIndices[current])
            }
            current = mArrayNextIndices[current]
            counter++
        }
    ...
    override operator fun get(v: SolverVariable?): Float {
        var current = head
        var counter = 0
        while (current != NONE && counter < currentSize) {
    ...
    override fun toString(): String {
        ...
        while (current != NONE && counter < currentSize) {
            ...
            result += mCache.mIndexedVariables.get(mArrayIndices[current])
            current = mArrayNextIndices[current]
            counter++
        }
        return result
