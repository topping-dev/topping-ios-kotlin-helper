...
class CLParser(private val mContent: String) {
    ...
    fun parse(): CLObject {
        ...
        for (i in startIndex + 1 until length) {
            ...
            if (currentElement?.isDone == true && (currentElement !is CLKey
                        || (currentElement as CLKey).mElements.size > 0)
            ) {
                currentElement = currentElement.container
            }
        }
    ...
    private fun createElement(
        ...
        if (currentElement is CLContainer) {
            val container: CLContainer = currentElement as CLContainer
