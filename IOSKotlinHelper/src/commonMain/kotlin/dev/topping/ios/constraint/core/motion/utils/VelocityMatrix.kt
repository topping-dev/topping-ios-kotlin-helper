...
class VelocityMatrix {
    ...
    fun applyTransform(
        ...
        dx += offx * mDScaleX
        dy += offy * mDScaleY
        val r = toRadians(mRotate.toDouble()).toFloat()
        val dr = toRadians(mDRotate.toDouble()).toFloat()
        dx += dr * (-width * offx * sin(r) - height * offy * cos(r)).toFloat()
        dy += dr * (width * offx * cos(r) - height * offy * sin(r)).toFloat()
        mAnchorDpDt[0] = dx
        mAnchorDpDt[1] = dy
    }
