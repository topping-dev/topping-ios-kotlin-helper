...
abstract class SplineSet {
    ...
    operator fun get(t: Float): Float {
        return (mCurveFit?.getPos(t.toDouble(), 0) ?: 0.0).toFloat()
    }

    // @TODO: add description
    fun getSlope(t: Float): Float {
        return (mCurveFit?.getSlope(t.toDouble(), 0) ?: 0.0).toFloat()
    }
    ...
    open fun setPoint(position: Int, value: Float) {
        if (mTimePoints.size < mCount + 1) {
            mTimePoints = Arrays.copyOf(mTimePoints, mTimePoints.size * 2)
            mValues = Arrays.copyOf(mValues, mValues.size * 2)
        }
    ...
    class CustomSet constructor(
        attribute: String,
        attrList: KeyFrameArray.CustomArray
    ) : SplineSet() {
    ...
    private class CoreSpline internal constructor(
        var mType: String,
        var mStart: Long
    ) : SplineSet() {
        
        override fun setProperty(widget: TypedValues, t: Float) {
            val id: Int = widget.getId(mType)
            widget.setValue(id, get(t))
        }
    }

    class CustomSpline constructor(
        attribute: String,
        attrList: KeyFrameArray.CustomVar
    ) : SplineSet() {
        var mAttributeName: String
