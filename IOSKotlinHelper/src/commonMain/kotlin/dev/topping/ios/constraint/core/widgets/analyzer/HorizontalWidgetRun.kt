...
class HorizontalWidgetRun(widget: ConstraintWidget) : WidgetRun(widget) {
    ...
    override fun supportsWrapComputation(): Boolean {
        return if (super.mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
    ...
    override fun apply() {
        ...
        if (!mDimension.resolved) {
            ...
            if (super.mDimensionBehavior != ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                if (mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_PARENT) {
                ...
                if (mDimensionBehavior == ConstraintWidget.DimensionBehaviour.FIXED) {
                    mDimension.resolve(mWidget.width)
                }
            }
        } else {
        ...
        if (mDimension.resolved && mWidget.measured) {
            ...
            if (mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
                when (mWidget.mMatchConstraintDefaultWidth) {
                    ...
                    ConstraintWidget.MATCH_CONSTRAINT_PERCENT -> {
                        ...
                        val parent: ConstraintWidget = mWidget.parent ?: return
    ...
    override fun update(dependency: Dependency?) {
                        ...
                        if (mWidget.mMatchConstraintDefaultHeight == ConstraintWidget.MATCH_CONSTRAINT_SPREAD
                            ...
                            if (s1 && s2 && e1 && e2) {
                                ...
                                if (secondStart.resolved && secondEnd.resolved) {
                                    ...
                                    val y1: Int = secondStart.value + secondStart.mMargin
                                    val y2: Int = secondEnd.value - secondEnd.mMargin
                                    computeInsetRatio(
                                        sTempDimensions,
                                ...
                                if (start.resolved && end.resolved) {
                                    ...
                                    val y1: Int = (secondStart.mTargets[0].value
                                            + secondStart.mMargin)
                                    val y2: Int =
                                        secondEnd.mTargets[0].value - secondEnd.mMargin
                                    computeInsetRatio(
                                        sTempDimensions,
                                        x1, x2, y1, y2, ratio, definedSide
                                    )
                                ...
                                val x2: Int = end.mTargets[0].value - end.mMargin
                                val y1: Int =
                                    secondStart.mTargets[0].value + secondStart.mMargin
                                ...
                                when (definedSide) {
                                    ConstraintWidget.UNKNOWN, ConstraintWidget.HORIZONTAL -> {
                                        val dx = x2 - x1
                                        var ldx: Int = getLimitedDimension(
                                            dx,
                                            ConstraintWidget.HORIZONTAL
                                        )
                                        val dy = (0.5f + ldx * ratio).toInt()
                                        val ldy: Int = getLimitedDimension(
                                            dy,
                                            ConstraintWidget.VERTICAL
                                        )
                                        if (dy != ldy) {
                                ...
                                when (definedSide) {
                                    ConstraintWidget.UNKNOWN, ConstraintWidget.VERTICAL -> {
                                        val dy = y2 - y1
                                        var ldy: Int = getLimitedDimension(
                                            dy,
                                            ConstraintWidget.VERTICAL
                                        )
                                        val dx = (0.5f + ldy / ratio).toInt()
                                        val ldx: Int = getLimitedDimension(
                                            dx,
                                            ConstraintWidget.HORIZONTAL
                                        )
                                        if (dx != ldx) {
        ...
        if (!mDimension.resolved
            && mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT && mWidget.mMatchConstraintDefaultWidth == ConstraintWidget.MATCH_CONSTRAINT_SPREAD && !mWidget.isInHorizontalChain
        ) {
        ...
        if (!mDimension.resolved
            && mDimensionBehavior == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT && matchConstraintsType == ConstraintWidget.MATCH_CONSTRAINT_WRAP
        ) {
