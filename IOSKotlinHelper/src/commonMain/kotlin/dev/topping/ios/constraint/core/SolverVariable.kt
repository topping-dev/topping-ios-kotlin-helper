...
class SolverVariable : Comparable<SolverVariable?> {
    ...
    var mType: Type
    var mClientEquations: Array<ArrayRow?> = arrayOfNulls<ArrayRow>(16)
    ...
    var mInRows: MutableSet<ArrayRow?>? = if (VAR_USE_HASH) mutableSetOf() else null
    ...
    fun addToRow(row: ArrayRow) {
        if (VAR_USE_HASH) {
            ...
            if (mClientEquationsCount >= mClientEquations.size) {
                mClientEquations = Arrays.copyOf(mClientEquations, mClientEquations.size * 2)
            }
    ...
    fun reset() {
        ...
        usageInRowCount = 0
        inGoal = false
        Arrays.fill(mGoalStrengthVector, 0f)
    }
    ...
    override operator fun compareTo(v: SolverVariable?): Int {
        return id - (v?.id ?: (id + 1))
    }
    ...
    override fun toString(): String {
        var result = ""
        if (INTERNAL_DEBUG) {
            result += name.toString() + "(" + id + "):" + strength
            if (mIsSynonym) {
                result += ":S($mSynonym)"
            }
            if (isFinalValue) {
    ...
    companion object {
        private val INTERNAL_DEBUG: Boolean = FULL_DEBUG
