 ...
 */
package dev.topping.ios.constraint.core.utils
...
import dev.topping.ios.constraint.core.widgets.VirtualLayout
...
class GridCore : VirtualLayout {
     ...
     */
    var mSpanIds: MutableSet<String?> = mutableSetOf()
     ...
     */
    var container: ConstraintWidgetContainer?
        get() = mContainer
        set(container) {
    ...
    private fun arrangeWidgets() {
        ...
        for (i in 0 until mWidgetsCount) {
            ...
            connectWidget(mWidgets.get(i), row, col, 1, 1)
        }
    }
    ...
    private fun parseWeights(size: Int, str: String?): FloatArray? {
        ...
        val values: Array<String> = str.split(",").toTypedArray()
        if (values.size != size) {
            return null
        }
        val arr = FloatArray(size)
        for (i in arr.indices) {
            arr[i] = values[i].trim().toFloat()
        }
        return arr
    }
    ...
    private fun makeNewWidget(): ConstraintWidget {
        ...
        widget.mListDimensionBehaviors[HORIZONTAL] = DimensionBehaviour.MATCH_CONSTRAINT
        widget.mListDimensionBehaviors[VERTICAL] = DimensionBehaviour.MATCH_CONSTRAINT
    ...
    private fun connectWidget(
        widget: ConstraintWidget?, row: Int, column: Int,
        rowSpan: Int, columnSpan: Int
    ) {
        // Connect the 4 sides
    ...
    private fun setBoxWidgetHorizontalChains() {
        ...
        var widget: ConstraintWidget? = mBoxWidgets!![0]
    ...
    private fun createBoxes() {
        ...
        if (mBoxWidgets == null) { // no box widgets build all
            mBoxWidgets = arrayOfNulls<ConstraintWidget>(boxCount)
            for (i in mBoxWidgets!!.indices) {
                mBoxWidgets!![i] = makeNewWidget() // need to remove old Widgets
            }
        } else {
            if (boxCount != mBoxWidgets!!.size) {
                val temp: Array<ConstraintWidget?> = arrayOfNulls<ConstraintWidget>(boxCount)
                ...
                for (j in boxCount until mBoxWidgets!!.size) {
                    val widget: ConstraintWidget? = mBoxWidgets!![j]
    ...
    private fun parseSpans(str: String?, isSpans: Boolean): Array<IntArray>? {
        return try {
            var extraRows = 0
            var extraColumns = 0
            val spans: Array<String> = str!!.split(",").toTypedArray()
            val spanMatrix = Array(spans.size) { IntArray(3) }
            var indexAndSpan: Array<String>
            if (mRows == 1 || mColumns == 1) {
                for (i in spans.indices) {
                    indexAndSpan = spans[i].trim().split(":").toTypedArray()
    ...
    private fun initMatrices() {
        val isUpdate =
            mConstraintMatrix != null && mConstraintMatrix!!.size == mWidgetsCount && mPositionMatrix != null && mPositionMatrix!!.size == mRows && mPositionMatrix!![0].size == mColumns
        if (!isUpdate) {
            initVariables()
        }
