...
abstract class WidgetRun(var mWidget: ConstraintWidget) : Dependency {
    ...
    var mDimension: DimensionDependency = DimensionDependency(this)
    var orientation = ConstraintWidget.HORIZONTAL
    var isResolved = false
    var start: DependencyNode = DependencyNode(this)
    var end: DependencyNode = DependencyNode(this)
    ...
    val isCenterConnection: Boolean
        get() {
            var connections = 0
            var count: Int = start.mTargets.size
            for (i in 0 until count) {
                val dependency: DependencyNode = start.mTargets.get(i)
            ...
            for (i in 0 until count) {
                val dependency: DependencyNode = end.mTargets.get(i)
                if (dependency.mRun != this) {
                    connections++
                }
            }
    ...
    protected fun getTarget(anchor: ConstraintAnchor): DependencyNode? {
        ...
        val targetWidget: ConstraintWidget = anchor.target!!.mOwner
    ...
    protected fun updateRunCenter(
        ...
        val startTarget: DependencyNode? = getTarget(startAnchor)
        val endTarget: DependencyNode? = getTarget(endAnchor)
        if (!(startTarget?.resolved == false && endTarget?.resolved == false)) {
            return
        }
        var startPos: Int = startTarget.value + startAnchor.margin
        var endPos: Int = endTarget.value - endAnchor.margin
