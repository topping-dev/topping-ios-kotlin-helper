...
"""
    ...
    private fun calcY(time: Float): Float {
        var timePar = time
        mDone = false
        if (timePar <= mStage1Duration) {
            return mStage1Velocity * timePar + ((mStage2Velocity - mStage1Velocity)
                    * timePar * timePar) / (2 * mStage1Duration)
        }
        if (mNumberOfStages == 1) {
            return mStage1EndPosition
        }
        timePar -= mStage1Duration
        if (timePar < mStage2Duration) {
            return mStage1EndPosition + mStage2Velocity * timePar + (mStage3Velocity - mStage2Velocity) * timePar * timePar / (2 * mStage2Duration)
        }
        if (mNumberOfStages == 2) {
            return mStage2EndPosition
        }
        timePar -= mStage2Duration
        if (timePar <= mStage3Duration) {
            return mStage2EndPosition + mStage3Velocity * timePar - mStage3Velocity * timePar * timePar / (2 * mStage3Duration)
        }
        mDone = true
    ...
    private fun setup(
        ...
        if (velocity < 0) { // backward
            ...
            val peak_v = sqrt(maxAcceleration * totalDistance).toFloat()
        ...
        val peak_v = sqrt(maxAcceleration * distance + velocity * velocity / 2).toFloat()
        mStage1Duration = (peak_v - velocity) / maxAcceleration
        mStage2Duration = peak_v / maxAcceleration
        if (peak_v < maxVelocity) { // accelerate then decelerate
    ...
    class Decelerate : StopEngine {
        ...
        override fun getVelocity(time: Float): Float {
            return if (time > mDuration) {
                0f
            } else mInitialVelocity + mAcceleration * time.also { mLastVelocity = it }
        }
