...
import dev.topping.ios.constraint.core.SolverVariable.Companion.STRENGTH_EQUALITY
import dev.topping.ios.constraint.core.SolverVariable.Companion.STRENGTH_HIGH
import dev.topping.ios.constraint.core.SolverVariable.Companion.STRENGTH_HIGHEST
import dev.topping.ios.constraint.core.SolverVariable.Companion.STRENGTH_LOW
import dev.topping.ios.constraint.core.SolverVariable.Companion.STRENGTH_MEDIUM
...
open class ArrayRow : LinearSystem.Row {
    ...
    fun toReadableString(): String {
        ...
        for (i in 0 until count) {
            val v: SolverVariable = variables!!.getVariable(i) ?: continue
            var amount = variables!!.getVariableValue(i)
            if (amount == 0f) {
                continue
            }
            val name: String = v.toString()
            if (!addedVariable) {
                if (amount < 0) {
                    s += "- "
                    amount *= -1f
    ...
    fun chooseSubjectInVariables(system: LinearSystem): SolverVariable? {
        ...
        val currentSize = variables?.currentSize ?: 0
        for (i in 0 until currentSize) {
            val amount = variables!!.getVariableValue(i)
            val variable: SolverVariable? = variables!!.getVariable(i)
        ...
        return if (unrestrictedCandidate != null) {
            unrestrictedCandidate
        } else restrictedCandidate
    }
    ...
    private fun isNew(variable: SolverVariable?, system: LinearSystem): Boolean {
        ...
        if (FULL_NEW_CHECK) {
            var isNew = true
            for (i in 0 until system.numRows) {
                val row: ArrayRow? = system.mRows!!.get(i)
                if (row?.hasVariable(variable) == true) {
                    isNew = false
                }
            }
    ...
    override fun updateFromRow(
        ...
        if (LinearSystem.SIMPLIFY_SYNONYMS
            && mVariable != null && variables!!.currentSize == 0
        ) {
            mIsSimpleDefinition = true
    ...
    private fun pickPivotInVariables(
        ...
        for (i in 0 until currentSize) {
            ...
            if (currentValue < 0) {
                ...
                val v: SolverVariable? = variables!!.getVariable(i)
                if(v == null)
                    continue
        ...
        return if (pivot != null) pivot else pivotSlack
    }
    ...
    override fun initFromRow(row: LinearSystem.Row?) {
        if (row is ArrayRow) {
            ...
            mVariable = null
            variables?.clear()
            val size = copiedRow.variables?.currentSize ?: 0
    ...
    override fun updateFromSystem(system: LinearSystem?) {
        ...
        while (!done) {
            ...
            for (i in 0 until currentSize) {
                val variable: SolverVariable? = variables!!.getVariable(i)
                if (variable != null && (variable.mDefinitionId != -1 || variable.isFinalValue || variable.mIsSynonym)) {
